...
> **Порт 80 конфликтует с работой №3.** Балансировщик Nginx из предыдущей работы уже
> занимает этот порт на `node1`. Перед запуском стека либо остановите Nginx
> (`sudo systemctl stop nginx`), либо опубликуйте стек на другом порту:
> `export T2_PORT=8080`.
...
Скрипт при первом запуске сам генерирует пароль базы и ключ подписи сессий, сохраняет
их в `deploy/lab4/.t2-secrets` и переиспользует при следующих запусках. Файл добавлен
в `.gitignore`. Без этих переменных стек намеренно не разворачивается: значений
по умолчанию в файле стека нет.
...
> **Выводите из строя веб-ноду, а не ту, где стоит метка хранилища.** База развёрнута
> в одной реплике с локальным томом, и при выключении её ноды сервис не поднимется
> на другой: том останется на выключенной машине. В реестре рисков из работы №1 мерой
> против этого записана репликация PostgreSQL, но в рамках учебного стека она
> не разворачивается. Это осознанный пропуск, а не упущение, и его стоит назвать вслух
> при защите.
